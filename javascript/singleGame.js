...
let isAIPlayer = {}; // Track which players are AI controlled
...
// AI Logic Functions
function getAIMove(playerNumber) {
  const heroType = selectedHeroes[playerNumber];
  const currentCell = document.querySelector(`.occupied-cell[data-player="P${playerNumber}"]`);
  if (!currentCell) return null;

  const cellIndex = Array.from(document.querySelectorAll('.game-cell')).indexOf(currentCell);
  ...
  directions.forEach(dir => {
    for (let step = 1; step <= movementRange; step++) {
      ...
      if (newX < 1 || newX > boardSize || newY < 1 || newY > boardSize) continue;
      ...
      if (!cell.classList.contains('occupied-cell')) {
        validMoves.push({ cell, x: newX, y: newY });
      } else {
        break;
      }
    }
  ...
  document.querySelectorAll('.occupied-cell').forEach(cell => {
    if (cell.getAttribute('data-player') !== `P${playerNumber}`) {
      const enemyIndex = Array.from(document.querySelectorAll('.game-cell')).indexOf(cell);
      const enemyY = Math.floor(enemyIndex / boardSize) + 1;
      const enemyX = (enemyIndex % boardSize) + 1;
      
      const distance = Math.abs(x - enemyX) + Math.abs(y - enemyY);
      if (distance < minDistance) {
        minDistance = distance;
        closestEnemy = { cell, x: enemyX, y: enemyY };
      }
    }
  });
  ...
  if (closestEnemy) {
    const bestMove = validMoves.reduce((best, move) => {
      const currentDist = Math.abs(move.x - closestEnemy.x) + Math.abs(move.y - closestEnemy.y);
      const bestDist = Math.abs(best.x - closestEnemy.x) + Math.abs(best.y - closestEnemy.y);
  ...
  // If no enemy found, make random move
  return validMoves[Math.floor(Math.random() * validMoves.length)].cell;
}

function getAIAction(playerNumber) {
  const heroType = selectedHeroes[playerNumber];
  const currentCell = document.querySelector(`.occupied-cell[data-player="P${playerNumber}"]`);
  if (!currentCell) return null;
  ...
  const attackTargets = findAdjacentTargets(x, y, playerNumber);
  if (attackTargets.length > 0) {
    // If health is low, consider defending
    if (playerHealth[playerNumber] < 40 && !playerDefense[playerNumber]) {
      return 'Defend';
    }
    
    // If special is ready and health is good, use it
    if (playerSpecialCooldowns[playerNumber] === 0 && playerHealth[playerNumber] > 60) {
      return 'Special';
    }
    
    // Otherwise attack
    return Math.random() < 0.7 ? 'Quick' : 'Heavy';
  }

  // If no targets in range, move
  return 'Move';
}

function getAITarget(playerNumber) {
  const currentCell = document.querySelector(`.occupied-cell[data-player="P${playerNumber}"]`);
  if (!currentCell) return null;
  ...
  document.querySelectorAll('.occupied-cell').forEach(cell => {
    if (cell.getAttribute('data-player') !== `P${playerNumber}`) {
      const targetIndex = Array.from(document.querySelectorAll('.game-cell')).indexOf(cell);
      const targetY = Math.floor(targetIndex / boardSize) + 1;
      const targetX = (targetIndex % boardSize) + 1;
      
      const distance = Math.abs(x - targetX) + Math.abs(y - targetY);
      targets.push({ cell, distance, health: playerHealth[cell.getAttribute('data-player').substring(1)] });
    }
  });
  ...
  return targets.sort((a, b) => a.health - b.health)[0].cell;
}
...
function generatePlayerCards(players) {
  ...
  for (let i = 1; i <= players; i++) {
    ...
    // If it's an AI player (player 2 or higher), automatically select a hero
    if (i > 1) {
      const heroTypes = ['knight', 'ninja', 'wizard'];
      const randomIndex = Math.floor(Math.random() * heroTypes.length);
      const randomHero = heroTypes[randomIndex];
      selectedHeroes[i] = randomHero;
      
      // Log the random selection for debugging
      console.log(`AI Player ${i} hero selection:`, {
        randomIndex,
        randomHero,
        allHeroes: heroTypes
      });
      ...
      playerCard.innerHTML = `
        ...
        <div class="hero-selection">
          <img src="../assets/chevalier.png" alt="knight" class="hero-image ${randomHero === 'knight' ? 'selected-hero' : ''}" style="opacity: ${randomHero === 'knight' ? '1' : '0.5'}">
          <img src="../assets/ninja.png" alt="ninja" class="hero-image ${randomHero === 'ninja' ? 'selected-hero' : ''}" style="opacity: ${randomHero === 'ninja' ? '1' : '0.5'}">
          <img src="../assets/sorcier.png" alt="wizard" class="hero-image ${randomHero === 'wizard' ? 'selected-hero' : ''}" style="opacity: ${randomHero === 'wizard' ? '1' : '0.5'}">
        </div>
      `;
    } else {
      // Human player gets to choose their hero
      playerCard.innerHTML = `
        <h4>Player ${i}</h4>
        <div class="hero-selection">
...
function startGame() {
  ...
  // Set up AI players (all players except player 1 are AI)
...
function promptNextPlayerInitiative(playerIndex, totalPlayers) {
  ...
  // If it's an AI player, automatically roll after a short delay
  if (isAIPlayer[playerNumber]) {
    setTimeout(() => {
  ...
  // Human player's turn to roll
  Swal.fire({
    title: `Player ${playerNumber}'s Turn to Roll`,
...
function rollInitiativeDice(playerIndex, totalPlayers) {
  ...
  setTimeout(() => {
    ...
    // Update game log
    addLogEntry(`Player ${playerNumber} rolled a ${randomNumber} for initiative!`);
...
function updateUIForCurrentPlayer() {
  ...
  const heroStats = {
    ...
    ninja: { health: 120, special: 'Double Attack' },
    wizard: { health: 120, special: 'Magic Storm (Area)' },
  };

  const hero = heroStats[selectedHero];
  const specialCooldown = playerSpecialCooldowns[playerNumber] || 0;
  ...
  actionPanel.innerHTML = `
    <h3>Actions</h3>
    <div class="current-player">
      <h4>Current Player: Player ${playerNumber}${isAIPlayer[playerNumber] ? ' (AI)' : ''}</h4>
    ...
    <div>
        ...
        <div class="actions">
            <button onclick="action('Move')" ${isAIPlayer[playerNumber] ? 'disabled' : ''}>
            ...
            <button onclick="action('Defend')" ${isAIPlayer[playerNumber] ? 'disabled' : ''}>
            ...
            <button onclick="action('Quick')" ${isAIPlayer[playerNumber] ? 'disabled' : ''}>
            ...
            <button onclick="action('Heavy')" ${isAIPlayer[playerNumber] ? 'disabled' : ''}>
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-sword"><polyline points="14.5 17.5 3 6 3 3 6 3 17.5 14.5"></polyline><line x1="13" x2="19" y1="19" y2="13"></line><line x1="16" x2="20" y1="16" y2="20"></line><line x1="19" x2="21" y1="21" y2="19"></line></svg>
                Heavy Attack
            </button>
            <button style="grid-column: span 2;" onclick="action('Special')" ${isAIPlayer[playerNumber] ? 'disabled' : ''}>
  ...
  // If it's an AI player's turn, execute their move after a short delay
  if (isAIPlayer[playerNumber]) {
    setTimeout(() => {
      executeAITurn(playerNumber);
    }, 1000);
  }
}

// Add new function to execute AI turns
function executeAITurn(playerNumber) {
  const action = getAIAction(playerNumber);
  addLogEntry(`AI Player ${playerNumber} chose to ${action}`);
  
  if (action === 'Move') {
    const moveCell = getAIMove(playerNumber);
    if (moveCell) {
      movePlayerToCell({ currentTarget: moveCell });
    } else {
      endTurn();
    }
    ...
    action('Special');
  } else if (action === 'Quick' || action === 'Heavy') {
    selectedAction = action;
    const target = getAITarget(playerNumber);
    if (target) {
      attackPlayer(target);
    } else {
      endTurn();
...
function attackPlayer(targetCell) {
  ...
  // If it's an AI player, automatically roll the dice after a short delay
  if (isAIPlayer[playerNumber]) {
    setTimeout(() => {
      rollBtn.click();
    }, 1000);
...
function movePlayerToCell(event) {
  ...
  if (targetCell.hasAttribute('data-attack-move')) {
    ...
    // If it's an AI player, they might want to attack after moving
    if (isAIPlayer[playerNumber]) {
      const attackTargets = findAdjacentTargets(newX, newY, playerNumber);
      if (attackTargets.length > 0) {
        const target = getAITarget(playerNumber);
        if (target) {
          setTimeout(() => {
            selectedAction = Math.random() < 0.7 ? 'Quick' : 'Heavy';
            attackPlayer(target);
          }, 1000);
          return;
        }
